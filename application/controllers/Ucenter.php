...
class Ucenter extends MJ_Controller {
    ...
    public function _init()
    {
        ...
        $this->load->model('mall_coupon_model', 'mall_coupon');
    ...
    public function index()
    {
        ...
        if (!$frontUserInfo) {
            ...
            $coupon_num = $this->mall_coupon->total(array('uid'=>$this->uid));
        ...
        $data['order'] = $this->mall_order_base->findById($where)->result();
        $orderid_arr = array();
    ...
    {
        $data['status_arr'] = array('1'=>'取消订单', '2'=>'未付款', '3'=>'已付款', '4'=>'已发货', '5'=>'已收货', '6'=>'已评价');
        $data['order'] = $this->mall_order_base->findById(array('order_id'=>$order_id))->row();
        $data['order_product'] = $this->mall_order_product->findById(array('order_id'=>$order_id))->result();
    ...
    public function pay_points()
    {
        ...
        $data['points_list'] = $this->account_log->findById(array('uid'=>$this->uid, 'account_type'=>2))->result();
        $frontUserInfo = unserialize(base64_decode(get_cookie('frontUserInfo')));
        $data['user_info'] = $frontUserInfo;
        $data['cms_block'] = $this->cms_block->findByBlockIds(array('foot_recommend_img','foot_speed_key'));
        $data['category'] = $this->help_category->getResultByFlag($flag=1);//左边栏显示
